...
router.post("/rounds", (req, res) => {
  ...
  if (!validatePlayerName(playerName)) {
    return res.status(400).json({
      error:
        "Invalid player name. Must be 2-16 alphanumeric characters or underscores.",
    });
  }

  if (!validateRoundsSurvived(roundsSurvived)) {
    return res.status(400).json({ error: "Invalid rounds survived." });
  }

  if (typeof bestWords !== "number" || bestWords < 0) {
    return res.status(400).json({ error: "Invalid best words." });
  }
...
router.get("/rounds", (req, res) => {
  ...
  try {
    ...
    const countStmt = db.prepare(
      "SELECT COUNT(*) as total FROM leaderboard_rounds",
    );
